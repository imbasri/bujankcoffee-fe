...
const ResetPassword = () => {
   ...
   const ResetPassword = (e) => {
      ...
      if (!otp || !password)
         return (
            toast.error("Data don't empty", {
               position: toast.POSITION.TOP_RIGHT,
            }),
            setLoading(false)
         );
      axios
         .patch(`${process.env.REACT_APP_BACKEND_HOST}/users/changePwd`, {
            otp,
            password,
         })
         .then((res) => {
         ...
         .catch((err) => {
            toast.error(
               "code otp expired please send email again in forgot password",
               {
                  position: toast.POSITION.TOP_RIGHT,
               }
            );
   ...
   titlebar("Bujank Coffee | Reset Password");
   return (
      <>
         <ToastContainer />
